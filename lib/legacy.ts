...
export function getLegacyPage(sourceFile: string): LegacyPageData {
  ...
  const rewritten = structureFooterLinks(
    protectBrandName(
      sanitizeLanguageAttributes(rewriteThemeLogos(rewriteImages(rewriteIframes(rewriteAnchors(withoutHero)))))
    )
  );

  const heroContentRewritten = heroContentHtml
    ? protectBrandName(sanitizeLanguageAttributes(rewriteAnchors(heroContentHtml)))
    : "";
